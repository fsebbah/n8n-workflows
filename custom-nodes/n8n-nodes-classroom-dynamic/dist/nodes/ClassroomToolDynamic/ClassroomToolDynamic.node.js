...
exports.ClassroomToolDynamic = void 0;
const n8n_workflow_1 = require("n8n-workflow");
class ClassroomToolDynamic {
    description = {
    ...
    async execute() {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            try {
                const accessToken = this.getNodeParameter('accessToken', itemIndex);
...
exports.ClassroomToolDynamic = ClassroomToolDynamic;
// === HELPER FUNCTION: Classroom API Request ===
async function classroomRequest(accessToken, method, endpoint, body, qs) {
    const options = {
        method: method,
...
async function classroomRequestAllPages(accessToken, endpoint, itemsKey, qs = {}, limit) {
    const allItems = [];
    let pageToken;
    do {
        ...
        const response = await classroomRequest.call(this, accessToken, 'GET', endpoint, undefined, queryParams);
...
// === COURSE OPERATIONS ===
async function executeCourseOperation(accessToken, operation, itemIndex) {
    switch (operation) {
        case 'create': {
            const name = this.getNodeParameter('name', itemIndex);
            const section = this.getNodeParameter('section', itemIndex, '');
            const description = this.getNodeParameter('description', itemIndex, '');
            const room = this.getNodeParameter('room', itemIndex, '');
        ...
        case 'get': {
            const courseId = this.getNodeParameter('courseId', itemIndex);
        ...
        case 'getAll': {
            ...
            const courseState = this.getNodeParameter('courseState', itemIndex, '');
            const qs = {};
            if (courseState)
                qs.courseStates = courseState;
            if (returnAll) {
        ...
        case 'update': {
            ...
            if (courseState) {
                body.courseState = courseState;
                updateMask.push('courseState');
            }
            return classroomRequest.call(this, accessToken, 'PATCH', `/courses/${encodeURIComponent(courseId)}`, body, { updateMask: updateMask.join(',') });
        }
        case 'delete': {
            const courseId = this.getNodeParameter('courseId', itemIndex);
        ...
        case 'archive': {
            const courseId = this.getNodeParameter('courseId', itemIndex);
            return classroomRequest.call(this, accessToken, 'PATCH', `/courses/${encodeURIComponent(courseId)}`, { courseState: 'ARCHIVED' }, { updateMask: 'courseState' });
        }
        default:
...
async function executeCourseWorkOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    switch (operation) {
        case 'create': {
            const title = this.getNodeParameter('title', itemIndex);
            const courseWorkDescription = this.getNodeParameter('courseWorkDescription', itemIndex, '');
            const workType = this.getNodeParameter('workType', itemIndex);
            const maxPoints = this.getNodeParameter('maxPoints', itemIndex, 100);
            const dueDate = this.getNodeParameter('dueDate', itemIndex, '');
            const dueTime = this.getNodeParameter('dueTime', itemIndex, '');
            const topicId = this.getNodeParameter('topicId', itemIndex, '');
            const state = this.getNodeParameter('courseWorkState', itemIndex, 'PUBLISHED');
            const body = {
                title,
                workType,
                state,
            };
            if (courseWorkDescription)
                body.description = courseWorkDescription;
            if (maxPoints)
            ...
            return classroomRequest.call(this, accessToken, 'POST', `/courses/${encodeURIComponent(courseId)}/courseWork`, body);
        }
        case 'get': {
            const courseWorkId = this.getNodeParameter('courseWorkId', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `/courses/${encodeURIComponent(courseId)}/courseWork/${encodeURIComponent(courseWorkId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const courseWork = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/courseWork`, 'courseWork');
                return { courseWork };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const courseWork = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/courseWork`, 'courseWork', {}, limit);
                return { courseWork };
            }
        }
        case 'update': {
            ...
            const title = this.getNodeParameter('title', itemIndex, '');
            const courseWorkDescription = this.getNodeParameter('courseWorkDescription', itemIndex, '');
            const maxPoints = this.getNodeParameter('maxPoints', itemIndex, 0);
            const dueDate = this.getNodeParameter('dueDate', itemIndex, '');
            const dueTime = this.getNodeParameter('dueTime', itemIndex, '');
            const topicId = this.getNodeParameter('topicId', itemIndex, '');
            const state = this.getNodeParameter('courseWorkState', itemIndex, '');
            const body = {};
            const updateMask = [];
            if (title) {
                body.title = title;
            ...
            return classroomRequest.call(this, accessToken, 'PATCH', `/courses/${encodeURIComponent(courseId)}/courseWork/${encodeURIComponent(courseWorkId)}`, body, { updateMask: updateMask.join(',') });
        }
        case 'delete': {
            const courseWorkId = this.getNodeParameter('courseWorkId', itemIndex);
            await classroomRequest.call(this, accessToken, 'DELETE', `/courses/${encodeURIComponent(courseId)}/courseWork/${encodeURIComponent(courseWorkId)}`);
...
async function executeStudentSubmissionOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    const courseWorkId = this.getNodeParameter('courseWorkId', itemIndex);
    const basePath = `/courses/${encodeURIComponent(courseId)}/courseWork/${encodeURIComponent(courseWorkId)}/studentSubmissions`;
    switch (operation) {
        case 'get': {
            const submissionId = this.getNodeParameter('submissionId', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `${basePath}/${encodeURIComponent(submissionId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const studentSubmissions = await classroomRequestAllPages.call(this, accessToken, basePath, 'studentSubmissions');
                return { studentSubmissions };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const studentSubmissions = await classroomRequestAllPages.call(this, accessToken, basePath, 'studentSubmissions', {}, limit);
                return { studentSubmissions };
            }
        }
        case 'grade': {
            const submissionId = this.getNodeParameter('submissionId', itemIndex);
            const assignedGrade = this.getNodeParameter('assignedGrade', itemIndex);
            const draftGrade = this.getNodeParameter('draftGrade', itemIndex, 0);
            const body = { assignedGrade };
            const updateMask = ['assignedGrade'];
            if (draftGrade > 0) {
                body.draftGrade = draftGrade;
                updateMask.push('draftGrade');
            }
            return classroomRequest.call(this, accessToken, 'PATCH', `${basePath}/${encodeURIComponent(submissionId)}`, body, { updateMask: updateMask.join(',') });
        }
        case 'return': {
            const submissionId = this.getNodeParameter('submissionId', itemIndex);
            return classroomRequest.call(this, accessToken, 'POST', `${basePath}/${encodeURIComponent(submissionId)}:return`, {});
        }
        case 'modifyAttachments': {
            const submissionId = this.getNodeParameter('submissionId', itemIndex);
            // For now, just return the submission - full attachment support would require additional parameters
            return classroomRequest.call(this, accessToken, 'POST', `${basePath}/${encodeURIComponent(submissionId)}:modifyAttachments`, { addAttachments: [] });
        }
        default:
...
// === STUDENT OPERATIONS ===
async function executeStudentOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    switch (operation) {
        case 'create': {
            const userId = this.getNodeParameter('userId', itemIndex);
            return classroomRequest.call(this, accessToken, 'POST', `/courses/${encodeURIComponent(courseId)}/students`, { userId });
        }
        case 'get': {
            const userId = this.getNodeParameter('userId', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `/courses/${encodeURIComponent(courseId)}/students/${encodeURIComponent(userId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const students = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/students`, 'students');
                return { students };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const students = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/students`, 'students', {}, limit);
                return { students };
            }
        }
        case 'delete': {
            const userId = this.getNodeParameter('userId', itemIndex);
            await classroomRequest.call(this, accessToken, 'DELETE', `/courses/${encodeURIComponent(courseId)}/students/${encodeURIComponent(userId)}`);
...
// === TEACHER OPERATIONS ===
async function executeTeacherOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    switch (operation) {
        case 'create': {
            const userId = this.getNodeParameter('userId', itemIndex);
            return classroomRequest.call(this, accessToken, 'POST', `/courses/${encodeURIComponent(courseId)}/teachers`, { userId });
        }
        case 'get': {
            const userId = this.getNodeParameter('userId', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `/courses/${encodeURIComponent(courseId)}/teachers/${encodeURIComponent(userId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const teachers = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/teachers`, 'teachers');
                return { teachers };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const teachers = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/teachers`, 'teachers', {}, limit);
                return { teachers };
            }
        }
        case 'delete': {
            const userId = this.getNodeParameter('userId', itemIndex);
            await classroomRequest.call(this, accessToken, 'DELETE', `/courses/${encodeURIComponent(courseId)}/teachers/${encodeURIComponent(userId)}`);
            return { success: true, courseId, userId, action: 'removed' };
        }
        default:
...
async function executeAnnouncementOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    switch (operation) {
        case 'create': {
            const text = this.getNodeParameter('text', itemIndex);
            const state = this.getNodeParameter('announcementState', itemIndex, 'PUBLISHED');
            const scheduledTime = this.getNodeParameter('scheduledTime', itemIndex, '');
            const body = { text, state };
            if (scheduledTime)
                body.scheduledTime = scheduledTime;
            return classroomRequest.call(this, accessToken, 'POST', `/courses/${encodeURIComponent(courseId)}/announcements`, body);
        }
        case 'get': {
            const announcementId = this.getNodeParameter('announcementId', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `/courses/${encodeURIComponent(courseId)}/announcements/${encodeURIComponent(announcementId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const announcements = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/announcements`, 'announcements');
                return { announcements };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const announcements = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/announcements`, 'announcements', {}, limit);
                return { announcements };
            }
        }
        case 'update': {
            const announcementId = this.getNodeParameter('announcementId', itemIndex);
            const text = this.getNodeParameter('text', itemIndex, '');
            const state = this.getNodeParameter('announcementState', itemIndex, '');
            const scheduledTime = this.getNodeParameter('scheduledTime', itemIndex, '');
            const body = {};
            const updateMask = [];
            if (text) {
            ...
            return classroomRequest.call(this, accessToken, 'PATCH', `/courses/${encodeURIComponent(courseId)}/announcements/${encodeURIComponent(announcementId)}`, body, { updateMask: updateMask.join(',') });
        }
        case 'delete': {
            const announcementId = this.getNodeParameter('announcementId', itemIndex);
            await classroomRequest.call(this, accessToken, 'DELETE', `/courses/${encodeURIComponent(courseId)}/announcements/${encodeURIComponent(announcementId)}`);
...
// === TOPIC OPERATIONS ===
async function executeTopicOperation(accessToken, operation, itemIndex) {
    const courseId = this.getNodeParameter('courseId', itemIndex);
    switch (operation) {
        case 'create': {
            const name = this.getNodeParameter('topicName', itemIndex);
            return classroomRequest.call(this, accessToken, 'POST', `/courses/${encodeURIComponent(courseId)}/topics`, { name });
        }
        case 'get': {
            const topicId = this.getNodeParameter('topicIdParam', itemIndex);
            return classroomRequest.call(this, accessToken, 'GET', `/courses/${encodeURIComponent(courseId)}/topics/${encodeURIComponent(topicId)}`);
        }
        case 'getAll': {
            const returnAll = this.getNodeParameter('returnAll', itemIndex);
            if (returnAll) {
                const topics = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/topics`, 'topic');
                return { topics };
            }
            else {
                const limit = this.getNodeParameter('limit', itemIndex);
                const topics = await classroomRequestAllPages.call(this, accessToken, `/courses/${encodeURIComponent(courseId)}/topics`, 'topic', {}, limit);
                return { topics };
            }
        }
        case 'update': {
            ...
            if (name)
                body.name = name;
            return classroomRequest.call(this, accessToken, 'PATCH', `/courses/${encodeURIComponent(courseId)}/topics/${encodeURIComponent(topicId)}`, body, { updateMask: 'name' });
        }
        case 'delete': {
            const topicId = this.getNodeParameter('topicIdParam', itemIndex);
            await classroomRequest.call(this, accessToken, 'DELETE', `/courses/${encodeURIComponent(courseId)}/topics/${encodeURIComponent(topicId)}`);
