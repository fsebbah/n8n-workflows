...
## Réponse n8n aux points torah-bot

### 1. Complexité vs valeur réelle ✅ D'ACCORD

Point valide. On commence par le MVP et on itère.

**Action** : Ajouter un compteur `free_translation_requests` dans les métriques pour mesurer l'usage réel.

### 2. Edit message Discord ✅ EXCELLENTE IDÉE

On abandonne le polling. L'edit de message est la solution optimale :

```python
msg = await message.reply("⏳ Traduction en cours...")
result = await translate_with_source_search(text)
await msg.edit(embed=build_result_embed(result))
```

**Avantages** :
- Zéro requête supplémentaire
- UX fluide (le message se met à jour sous les yeux de l'utilisateur)
- Pas besoin d'endpoint `GET /batch/{job_id}`

**Impact architecture** : Simplification majeure. Plus besoin de :
- ~~Endpoint polling~~
- ~~Job queue status~~
- ~~Webhook callback~~

### 3. Embeddings hébreu ancien 🤔 REPORT v2

D'accord pour reporter Qdrant en v2. Le benchmark sur 100 textes est une bonne idée avant d'investir.

**Alternative v1** : `pg_trgm` (trigram) dans PostgreSQL.

### 4. Index substring : `pg_trgm` ✅ DÉCIDÉ

On utilise **pg_trgm** (trigram) pour la recherche fuzzy :

```sql
-- Extension
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Index trigram sur source_texts
CREATE INDEX idx_source_texts_text_trgm
ON source_texts USING gin (text gin_trgm_ops);

-- Recherche fuzzy
SELECT * FROM source_texts
WHERE text % 'הָעוֹשֶׂה לוּלָב'  -- similarité trigram
ORDER BY similarity(text, 'הָעוֹשֶׂה לוּלָב') DESC
LIMIT 5;
```

**Avantages pg_trgm** :
- Natif PostgreSQL (pas de service externe)
- Tolère les fautes de frappe / variantes
- Index GIN performant
- Fonctionne bien avec l'hébreu

**Seuil de similarité** : `similarity > 0.3` (à ajuster après tests)

### 5. Sefaria : reporté v3 ✅ D'ACCORD

Si torah.api a déjà les textes, Sefaria est redondant. On le garde en fallback v3 uniquement pour les textes non présents en base.

### 6. Timeout 15s ✅ D'ACCORD

15s max est raisonnable pour Discord. Workflow :

```
T+0s   : "⏳ Traduction en cours..."
T+3s   : Edit → Traduction affichée
T+3-15s: Edit → Source ajoutée (si trouvée)
T+15s  : Abandon recherche source si non trouvée
```

### 7. Champs supplémentaires ✅ D'ACCORD
...
ADD COLUMN source_method VARCHAR(20),  -- 'exact', 'trigram', 'qdrant', 'sefaria', 'llm'
ADD COLUMN source_verified BOOLEAN DEFAULT FALSE;
```

### 8. Questions manquantes - Réponses

| Question | Réponse |
|----------|---------|
| **Coût** | v1 (pg_trgm) = gratuit. v2 (Qdrant) = hosting + embeddings |
| **Rétrocompatibilité** | Les anciennes `free_translations` gardent `source_text_id = NULL` |
| **Feedback loop** | Bouton "❌ Mauvaise source" → `source_verified = FALSE` + log pour review |
| **Rate limiting** | Limiter à 5 req/min/user pour les textes libres |

---

## Consensus final : MVP v1
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Discord: "⏳ Traduction en cours..."                        │
│                                                              │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ 0. Normalisation Unicode NFC                           │ │
│  │ 1. Hash exact (PostgreSQL)                             │ │
│  │ 2. Recherche trigram pg_trgm (PostgreSQL)              │ │
│  │ 3. Traduction LLM (en parallèle avec 1-2)              │ │
│  └────────────────────────────────────────────────────────┘ │
│                                                              │
│  Discord: Edit message → Traduction + Source (si trouvée)   │
│                                                              │
│  Timeout: 15s max                                            │
│                                                              │
└─────────────────────────────────────────────────────────────┘
...
**Stack v1** :
- PostgreSQL + pg_trgm (pas de Qdrant)
- Discord edit message (pas de polling/webhook)
- Celery pour async (optionnel si < 15s)
...
|---|----------|---------|----------|
| 1 | Index substring | `tsvector` / `LIKE` / `trigram` | ✅ **pg_trgm** |
| 2 | Seuil trigram | 0.2 / 0.3 / 0.4 | ⏳ 0.3 (à tester) |
| 3 | Seuil Qdrant (v2) | 0.85 / 0.90 / 0.95 | ⏳ Reporté v2 |
| 4 | Stocker confidence_score | Oui / Non | ✅ **Oui** |
| 5 | Timeout global | 15s / 30s / 60s | ✅ **15s** |
| 6 | Notification Discord | Polling / WebSocket / Edit | ✅ **Edit message** |

---

## Prochaines étapes MVP v1

### torah.api
- [ ] Implémenter normalisation Unicode (NFC)
- [ ] Activer extension `pg_trgm`
- [ ] Créer index trigram sur `source_texts`
- [ ] Migration `free_translations` (4 nouveaux champs)
- [ ] Endpoint recherche source par trigram

### torah-bot
- [ ] Implémenter edit message Discord
- [ ] Timeout 15s sur recherche source
- [ ] Affichage enrichi (traduction + source + confidence)

### n8n
- [ ] Workflow traduction avec recherche source parallèle
- [ ] Métriques `free_translation_requests`

### Futures versions
- [ ] **v2** : Benchmark embeddings hébreu + Qdrant
- [ ] **v3** : Fallback Sefaria/LLM si nécessaire

---

_Mis à jour : 2024-12-29_
